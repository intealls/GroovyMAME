...
GroovyMAME with emusync and PART can provide latencies comparable to running on
a real system, but might require some additional effort to get everything
running smoothly.

## System requirements

* At least a 4th generation Intel i5 4-core or similar is recommended for good
  performance.
* At least 4 GB RAM.
...
* If you're having problems on Linux with sudden tearing, disable NTP or sync
  once on boot before launching GM.
...
On Linux, the operating system clock is usually synchronized with NTP. If you're
having problems with sudden (unexplainable) tearing, disable NTP on your system,
or synchronize once on boot before launching GM.
...
other open applications and try launching MAME directly.
...
When initializing GM (when started with -sound part), the info log will output
something like this:
...
(default `-audio_latency`) is reported to be around ~18.67 ms + 2.0 ms =
~20.67ms.
...
For PART, the main source of latency is the resulting latency of the frequency
at which the operating system forwards audio to the audio device (given you're
using analog, wired, on-board audio). For PART, this is controlled with the
`-audio_latency` parameter (specified in milliseconds). This is almost always
the source of crackling audio and might require some trial and error to figure
out what the system is capable of. The Linux/Windows sections detail how to
...
Start out with `-audio_latency 4`, increase to `8` and then `16` if you notice
audio crackles.
...
Use the `-part_device` parameter to force a specific device if desired, the
reported latency numbers are put in the verbose log (like above).
...
* Use `-nosleep` for best performance, this might increase power consumption /
  heat but helps out with both emusync and PART.
...
* The `-audio_latency` parameter for PART is specified in milliseconds. Check
  the verbose log if you are unsatisfied with the defaults. On Windows the
  defaults work well with on-board audio.
